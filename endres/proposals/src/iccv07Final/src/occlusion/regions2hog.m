function hog = regions2hog(regions, bndinfo, pb1, pb2, norient, hogSize)
...
for k = 1:bndinfo.ne
    ew(k) = numel(bndinfo.edges.indices{k});
end
ew1 = ew.*pb1;
ew2 = ew.*pb2;
ew12 = ew1+ew2;
...
%    end
    left = ismember(edges.spLR(:, 1), regions{k});
    right = ismember(edges.spLR(:, 2), regions{k});
...
for k = 1:size(hogSize, 1)
    ...
    for b = 1:nb                       
        hog(c+b) = sum(ew(indo==b)) / (sum(ew_norm(ind==ceil(b/norient)))+0.01);
