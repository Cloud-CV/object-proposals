function [vacc, hacc, vcm, hcm, pg] = crfTestCV2(imsegs, pvSP, phSP, adjlist, pE, crfw, param, edgelen)
...
function pg = crfTestImage(pvSP, phSP, adjlist, pE, crfw, param, edgelen)
...
for k = 1:numel(s1)
    adjmat(s1(k), s2(k)) = 1;    
    adjmat(s2(k), s1(k)) = 1;    
    potentials{s1(k), s2(k)} = 0.5*squeeze(exp(crfw(1)*f1(k, :, :) + crfw(2)*f2(k, :, :) + crfw(3)*f3(k, :, :)));
