...
static void savePPM_map(image<rgb> *im, int seg, image<int> *segs, char *name) {
   rgb black;
   memset((char*)&black, 0, sizeof(rgb));
   int i,j;
  int width = im->width();
  int height = im->height();
  std::ofstream file(name, std::ios::out | std::ios::binary);
  ...
  file << "P6\n" << width << " " << height << "\n" << UCHAR_MAX << "\n";
   for(j = 0; j < height; j++) {
      for(i = 0; i < width; i++) {
         if(*imPtr(segs, i, j) == seg)
            file.write((char *)imPtr(im, i, j), sizeof(rgb));
         else 
            file.write((char *)&black, sizeof(rgb));
      }
   }
}
