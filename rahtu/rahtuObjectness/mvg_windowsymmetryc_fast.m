function [winscores]=mvg_windowsymmetryc_fast(img, windows)
...
BoxWeights=ones(2*nsubbox,2*nsubbox);
BoxWeights(2:(end-1),2:(end-1))=0.25*BoxWeights(2:(end-1),2:(end-1));
%BoxWeights

winscores=wsscores(windows,oribinintim,nbin,nsubbox,BoxWeights);
winscores=(winscores./max(winscores));
...
for i=1:nw
  ...
  %nedgepixels(i,1)=cedgeint(yb,xb)+cedgeint(yaa,xaa)-cedgeint(yaa,xb)-cedgeint(yb,xaa);
  %if nedgepixels(i,1)<0
  ...
  for j=1:nsubbox
    for k=1:nsubbox
      intersection=intersection+BoxWeights(j,k)*sum(min(winhists(j,:,k),winhists(j,[1 4 3 2],2*nsubbox+1-k)));
    end
  end
  winscores(i,1)=intersection;
  
  intersection=0;
  for j=1:nsubbox
    for k=1:nsubbox
      intersection=intersection+BoxWeights(j,k)*sum(min(winhists(j,:,k),winhists(2*nsubbox+1-j,[1 4 3 2],k)));
    end
  end
  winscores(i,2)=intersection;
  
  intersection=0;
  for j=(nsubbox+1):(2*nsubbox)
    for k=1:nsubbox
      intersection=intersection+BoxWeights(j,k)*sum(min(winhists(j,:,k),winhists(j,[1 4 3 2],2*nsubbox+1-k)));
    end
  end
  winscores(i,3)=intersection;

  intersection=0;
  for j=1:nsubbox
    for k=(nsubbox+1):(2*nsubbox)
      intersection=intersection+BoxWeights(j,k)*sum(min(winhists(j,:,k),winhists(2*nsubbox+1-j,[1 4 3 2],k)));
    end
  end
  winscores(i,4)=intersection;
  
end


winscoremaxsum=max(sum(winscores,2));
winscores=(winscores./winscoremaxsum);
winscores=sum(winscores,2);
end
