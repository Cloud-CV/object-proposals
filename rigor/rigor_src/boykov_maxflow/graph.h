...
public:
  ...
  typedef std::pair<tcaptype, tcaptype> NodeCap;
  typedef std::pair<node_id, NodeCap> SrcSeedNode;
  typedef std::vector<SrcSeedNode> SrcSeedList;
  typedef std::vector<SrcSeedList> FGSeedsType;
	...
	flowtype maxflow(bool reuse_trees = false, Block<node_id>* changed_list = NULL, bool run_init_func = true);
	...
	// This function takes a filename and generates a tex output file which
	// generates the current state of the graph using TikZ. The filename needs
	// to be without the .tex extension. The nodes are displayed in a grid like
	// fashion and are organized in a column major order. The NODE_ROWS gives
	// the number of nodes in each column
	void generate_graph_visualization(const unsigned int& NODE_ROWS,
	                                  const std::string& graph_filename=std::string("graph1"));

	// Takes all the tex files generated by generate_graph_visualization and
	// produces a single pdf. The list of graphs is maintained in
	// graphs_vis_generated
	void generate_pdf_graphs(const std::string& main_tex_filepath=std::string("boykov_graphs")) const;
...
public:
	...
	typedef arc* arc_id;
	arc_id get_first_arc() const;
	arc_id get_next_arc(arc_id a) const;

	// other functions for reading graph structure
	int get_node_num() const { return node_num; }
	int get_arc_num() const { return (int)(arc_last - arcs); }
	void get_arc_ends(arc_id a, node_id& i, node_id& j) const; // returns i,j to that a = i->j
	...
	tcaptype get_trcap(node_id i) const;
	// returns residual capacity of arc a
	captype get_rcap(arc* a) const;
  ...
  // This function is to be used after multiple seed graph is pre-computed.
  // The keep_src_id specifies which seed to keep (and all others are converted
  // to sink nodes). remove_seed_list gives the nodes which need to be used for
  // converting source seed trees to sink trees
  void transform_seed_trees(const int keep_src_id,
                            const SrcSeedList* remove_seed_list);

  void update_capacities(const size_t& num_params,
  ...
  void reparam_unary(const node_id& var_i,  const tcaptype& old_cap_diff,
                     const tcaptype& new_cap_diff);

  bool check_tree_integrity(const std::string& filename="") const;

  void set_node_source_idx(node_id i, const int& src_origin_idx)
  {
    assert(i>=0 && i<node_num);
    nodes[i].src_origin_idx = src_origin_idx;
  }

  int get_node_source_idx(node_id i) const
  {
    return nodes[i].src_origin_idx;
  }

  void print_changed_list() const
  {
    printf("List of nodes whose label might have changed: ");
    for (node* node_ptr = nodes; node_ptr < node_last; ++node_ptr) {
      if (node_ptr->is_in_changed_list) {
        unsigned int node_idx = node_ptr - nodes;
        printf("%d, ", node_idx);
      }
    }
    printf("\n");
  ...
  flowtype get_flow() const { return flow; }

  unsigned int get_num_growths() const { return num_growths; }
  unsigned int get_num_augmentations() const { return num_augmentations; }
  unsigned int get_num_adoptions() const { return num_adoptions; }

  int get_maxflow_iteration() const { return maxflow_iteration; }
...
private:
	...
	struct node
	{
		...
		arc			*parent;	// node's arc pointing to parent
	...
	struct arc
	{
		...
		arc() : head(NULL), next(NULL), sister(NULL) {}
	};
  ...
  typedef std::set<node*> NodeEditSet;

	int curr_src_origin_idx;
	...
	unsigned int num_growths, num_augmentations, num_adoptions;

	std::vector<std::string> graphs_vis_generated;   // list of graphs that were generated by visualize_graph()

  void normalize_tree(NodeEditSet& tree_nodes);
  void normalize_node(node* const n_ptr);

  void set_active_normalization();

	void test_consistency(node* current_node=NULL) const; // debug function

  void get_node_r_c(const node* const node_ptr, unsigned int& node_idx,
                    unsigned int& r, unsigned int& c, const int& NODE_ROWS) const;

  void gassert(bool true_cond, const char* msg) const;
  void gassert(bool true_cond, const std::string& msg) const;
};
...
template <typename captype, typename tcaptype, typename flowtype>
  inline void Graph<captype, tcaptype, flowtype>::gassert(bool true_cond,
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_first_arc() const
{
	return arcs;
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_next_arc(arc* a) const
{
	return a + 1; 
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::get_arc_ends(arc* a, node_id& i, node_id& j) const
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline tcaptype Graph<captype,tcaptype,flowtype>::get_trcap(node_id i) const
{
	assert(i>=0 && i<node_num);
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline captype Graph<captype,tcaptype,flowtype>::get_rcap(arc* a) const
{
	assert(a >= arcs && a < arc_last);
	return a->r_cap;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::termtype
	  Graph<captype,tcaptype,flowtype>::what_segment(node_id i, termtype default_segm) const
{
	if (nodes[i].parent)
	{
...
template <typename captype, typename tcaptype, typename flowtype>
  inline void Graph<captype,tcaptype,flowtype>::reparam_unary(const node_id& var_i,
                                                              const tcaptype& old_cap_diff,
                                                              const tcaptype& new_cap_diff)
{
  tcaptype residual_diff = new_cap_diff - old_cap_diff;

  /* if the residual capacity difference is positive then src capacity
   * needs to be increased. If the residual capacity difference is
   * negative then the sink capacity needs to be increased */
  /*
  if (residual_diff > 0) {
    add_tweights(var_i, residual_diff, 0);
    mark_node(var_i);
  } else if (residual_diff < 0) {
    add_tweights(var_i, 0, -residual_diff);
    mark_node(var_i);
  }*/

  //old_residual_cap = get_trcap(var_i);

  set_trcap(var_i, nodes[var_i].tr_cap + residual_diff);
  //mark_node(var_i);

  /*
  if (residual_diff < 0) {
    tcaptype new_residual_cap = get_trcap(var_i);
    // Mark node only if the sign of the residual capacity changed
    if (!(old_residual_cap >= 0 ^ new_residual_cap < 0)) {
      mark_node(var_i);
    }
  }
  */
...
// including templated definitions
#include "graph-impl.hpp"
#include "maxflow-impl.hpp"
#include "precomptrees-impl.hpp"
#include "visualize-impl.hpp"
