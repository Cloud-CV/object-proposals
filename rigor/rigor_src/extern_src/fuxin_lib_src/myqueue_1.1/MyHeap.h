...
private:
	 ...
	 */
	bool useBackIdx;
    pair<Tkey, int> empty_ind;
public:
	/// Simple constructor with NO cross updates
	MaxHeap(){
		useBackIdx = false;
        empty_ind = make_pair(Tkey(0),-1);
	}	
	...
	void push( Tkey key, int index ){
		...
		{
			//mexPrintf("The index is %d the size is %d", index, (int)backIdx.size());
...
//			throw InvalidKeyIncreaseException("In MaxHeaps only increase key updates are legal");
		...
		// Don't do anything if the key is unchanged
		if (key == heap[currIdx].first)
			return;
