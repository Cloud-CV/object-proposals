classdef ColorGraphFuxin < AbstractGraph
%COLORGRAPHFUXIN is a variant of ColorGraph, which generates graphs with 
% seeds and unaries using color information. This class is derived from
% AbstractGraph. To undestand how unaries are generated, its worth seeing 
% how AbstractGraph.prepare_graphs() calls create_unaries() in this class 
% to compute unary values.
%
% @authors:     Fuxin Li
...
% @date:        Fall 2013 - Summer 2014
    ...
    methods
        function obj = ColorGraphFuxin(seg_obj, segm_index)
        ...
        function create_unaries(obj, graph_sub_method, seed_sets)
            switch graph_sub_method
                ...
                case 'external2'
                    obj.generate_color_unaries('external2',seed_sets);
                case 'subframe'
        ...
        function generate_color_unaries(obj, internal_external, seed_sets)
            ...
            for i = 1:size(seed_sets, 2)
                ...
                elseif strcmp(internal_external,'external')
                    ...
                    [nonlambda_s, nonlambda_t, lambda_s, lambda_t] = ...
                        external_color2(obj, seed_sets(:,i), ...
                                       inside_frame, bg_unary_values, ...
                                       clr_unary_vals(:,i), ...
                                       funcs.parametric_func);
...
%           frame_cost = obj.frame_bg_bias(fg_seeds, size(obj.I));
        ...
        function [nonlambda_s, nonlambda_t, lambda_s, lambda_t] = ...
                external_color2(obj, fg_seeds, inside_frame, ...
                               bg_unary_values, fg_unary_values, ...
                               parametric_func)
            % see the comment for these output matrices in 
            % AbstractGraph.prepare_graphs()
...
%                 frame_cost*obj.SEED_FRAME_WEIGHT;
        ...
        function [bg_unary_values, clr_unary_vals] = ...
            ...
            seed_hash = num2str(sum(getByteStreamFromArray(seed_sets)));
...
%                                 obj.seg_obj.other_params.hash_params);
            ...
            precomputed_unary_idx = find(strcmp(seed_hash, ...
                                         obj.precompute.clr_unary_hash));
            % precompute the fg color unary values
            if isempty(precomputed_unary_idx)
                clr_unary_values = zeros(size(seed_sets));
                % iterate over all seeds - compute unary values for each sp
                for i = 1:size(seed_sets, 2)
                    clr_unary_values(:,i) = ...
