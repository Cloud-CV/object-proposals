classdef UniformGraphFuxin < AbstractGraph
%UNIFORMGRAPHFUXIN is a variant of UniformGraph, which generates graphs 
% with seeds and unaries which are not dependent on image information (for 
% instance unaries are only based on superpixel size). This class is 
% derived from AbstractGraph. To undestand how unaries are generated, its
% worth seeing how AbstractGraph.prepare_graphs() calls create_unaries() in 
% this class to compute unary values.
%
% @authors:     Fuxin Li
...
% @date:        Fall 2013 - Summer 2014
    ...
    methods
        function obj = UniformGraphFuxin(seg_obj, segm_index)
        ...
        function create_unaries(obj, graph_sub_method, seed_sets)
            switch graph_sub_method
                ...
                case 'external2'
                    obj.generate_uniform_unaries('external2', seed_sets);
                case 'subframe'
                    error('UniformGraphFuxin:compute_graph', ...
        ...
        function generate_uniform_unaries(obj, internal_external, ...
            ...
            for i = 1:size(seed_sets, 2)
                if strcmp(internal_external, 'internal')
                    [nonlambda_s, lambda_s, lambda_t] = ...
                        internal_uniform(obj, seed_sets(:,i), ...
                ...
                elseif strcmp(internal_external, 'external')
                    ...
                    [nonlambda_s, lambda_s, lambda_t] = ...
                        external_uniform2(obj, seed_sets(:,i), ...
                                          inside_frame, ...
                                          funcs.parametric_func, ...
                                          funcs.frame_cost_func);
        ...
        function [nonlambda_s, lambda_s, lambda_t] = ...
                internal_uniform(obj, the_rectangle, parametric_func, ...
                                 frame_cost_func)
...
% Internal is always without frame_cost
            nonlambda_s(the_rectangle) = inf;
            lambda_t(bground_pixels) = parametric_func(bground_pixels) / 2; %* obj.graph_seed_frame_weight;
            lambda_s(bground_pixels) = parametric_func(bground_pixels); %*obj.graph_seed_frame_weight;
        end
        ...
        function [nonlambda_s, nonlambda_t, lambda_s, lambda_t] = ...
            ...
            inside_frame_but_rect = ~the_rectangle;
            lambda_weights = parametric_func(inside_frame_but_rect); %* obj.graph_seed_frame_weight;
%             bground_pixels = ~(inside_frame | obj.seg_obj.sp_frame_set);
            ...
            nonlambda_t(obj.seg_obj.sp_frame_set) = ...
                frame_cost * 2 .* parametric_func(obj.seg_obj.sp_frame_set);
            lambda_t(inside_frame_but_rect) = lambda_weights / 256;
            % no frame edge if the frame superpixel is seed
            nonlambda_t(the_rectangle) = 0;
        end
        
        function [nonlambda_s, lambda_s, lambda_t] = ...
                external_uniform2(obj, the_rectangle, inside_frame, ...
                                  parametric_func, frame_cost_func)
            % see the comment for these output matrices in 
            % AbstractGraph.prepare_graphs()
            nonlambda_s = zeros(length(the_rectangle),1);
            ...
            lambda_s(inside_frame_but_rect) = lambda_weights;
            lambda_t(inside_frame_but_rect) = lambda_weights / 256;
            % TODO: Want to add horiz/vert, these 2 are useful.
        end
    end
end
